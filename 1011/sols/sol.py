...
for user_id, total_score in res.items():
    ...
    # Округлення значення до 2 значущих цифр після коми
    p_rounded = round(p, 2)
    # Приведення до рядка з двома значущими цифрами після коми
    p_str = "{:.2f}".format(p_rounded)
    print(f"{user_id}: {verdict(p_rounded)} [{p_str}%]")
